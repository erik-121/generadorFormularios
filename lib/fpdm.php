		...
		//------------------------------
			...
			if($this->support == "native") {
				...
				if($count_entries) {
					...
					//print_r($this->value_entries);
					/*foreach($this->value_entries as $clave=>$valor){
						print_r($clave." ");
					}*/
					if($this->verbose) {
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
		...
		*@internal note pack(\93C\94,hexdec(substr($data,$i,2))) DOES NOT WORK
		...
		*+	FlateDecode (PDF\A01.2): Decompresses data encoded using the zlib/deflate compression method, reproducing the original text or binary data.
		...
		*   JBIG2Decode (PDF\A01.4) :Decompresses data encoded using the JBIG2 standard, reproducing the original monochrome (1 bit per pixel) image data (or an approximation of that data).
